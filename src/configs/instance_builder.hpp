...
class ConfigInstanceBuilder {
 public:
  ...
  ConfigInstanceBuilder& with_config_schema_file(const miru::filesys::File& schema_file);
  ...
  ConfigInstanceBuilder& with_config_schema_digest(const std::string& schema_digest);
 ...
 private:
  // required
  std::optional<miru::filesys::File> config_schema_file_;
