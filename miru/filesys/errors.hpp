...
#define THROW_FILE_NOT_FOUND(path) \
    throw FileNotFoundError(path, ERROR_TRACE)

class NotAFileError : public std::runtime_error {
 public:
  explicit NotAFileError(
    const std::string& path,
    const errors::ErrorTrace& trace
  )
    : std::runtime_error(format_message(path, trace)) {}
  ...
  static std::string format_message(
    ...
    return "File '" + path + "' exists but is not a file" + errors::format_source_location(trace);
  }
};

#define THROW_NOT_A_FILE(path) \
    throw NotAFileError(path, ERROR_TRACE)
...
class DirNotFoundError : public std::runtime_error {
 public:
  ...
  static std::string format_message(
    ...
    return "Directory '" + path + "' does not exist" + errors::format_source_location(trace);
  }
};

#define THROW_DIR_NOT_FOUND(path) \
    throw DirNotFoundError(path, ERROR_TRACE)

class NotADirError : public std::runtime_error {
 public:
  explicit NotADirError(
    const std::string& path,
    const errors::ErrorTrace& trace
  )
    : std::runtime_error(format_message(path, trace)) {}
  ...
  static std::string format_message(
    ...
    return "Path '" + path + "' exists but is not a directory" + errors::format_source_location(trace);
  }
};

#define THROW_NOT_A_DIR(path) \
    throw NotADirError(path, ERROR_TRACE)
...
class UnableToFindGitRepoError : public std::runtime_error {
 public:
  ...
  static std::string format_message(
    ...
    return "The path '" + path + "' is not part of a git repository" + errors::format_source_location(trace);
  }
};
