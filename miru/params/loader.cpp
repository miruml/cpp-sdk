...
miru::params::Parameter load_json_array(
    ...
    switch (first_entry.type()) {
        ...
        case nlohmann::json::value_t::null: {
            throw std::runtime_error("Null values are not supported in arrays. Please contact Ben at ben@miruml.com if you need this feature.");
        }
        case nlohmann::json::value_t::boolean: {
        ...
        case nlohmann::json::value_t::array: {
            ...
            for (const auto& entry : node.items()) {
                ...
                entries.push_back(
                    load_json_array(entry_name, entry.value(), delimiter)
                );
                i++;
            }
        ...
        case nlohmann::json::value_t::object: {
            ...
            for (const auto& entry : node.items()) {
                if (entry.value().type() != nlohmann::json::value_t::object) {
                    throw std::runtime_error("Heterogeneous array types are not supported. Please contact Ben at ben@miruml.com if you need this feature.");
                }
                std::string entry_name = name + delimiter + std::to_string(i);
                entries.push_back(
                    load_json(entry_name, entry.value(), delimiter)
                );
                i++;
            }
            return miru::params::Parameter(name, entries);
...
miru::params::Parameter load_yaml_array(
    ...
    switch (first_entry.Type()) {
        ...
        case YAML::NodeType::Scalar: {
            ...
            return miru::params::Parameter(
                name,
                miru::params::ParameterValue(scalar_array)
            );
        }
        case YAML::NodeType::Sequence: {
            throw std::runtime_error("Nested arrays are not supported. Please contact Ben at ben@miruml.com if you need this feature.");
        }
        case YAML::NodeType::Map: {
            throw std::runtime_error("Nested objects are not supported. Please contact Ben at ben@miruml.com if you need this feature.");
        }
    }
    throw std::runtime_error("Unsupported node type");
}
    ...
    switch (node.Type()) {
        ...
        case YAML::NodeType::Map: {
            ...
            for (const auto& entry : node) {
                ...
                entries.push_back(
                    load_yaml(entry_name, entry.second)
                );
            }
            return miru::params::Parameter(
                name,
