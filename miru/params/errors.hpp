...
class InvalidParameterTypeError : public std::runtime_error {
 public:
  ...
  InvalidParameterTypeError(
    const std::string& name, const std::string& message, const errors::ErrorTrace& trace
  )
...
class InvalidScalarConversionError : public std::runtime_error {
 public:
  InvalidScalarConversionError(
    const std::string& value, const std::string& dest_type, const std::string& message,
...
class EmptyInitializationError : public std::invalid_argument {
 public:
  EmptyInitializationError(
    const std::string& object_to_initialize, const errors::ErrorTrace& trace
  )
...
class DuplicateFieldNamesError : public std::invalid_argument {
 public:
  DuplicateFieldNamesError(
    const std::string& object_to_initialize, const std::string& duplicate_field_name,
    const errors::ErrorTrace& trace
  )
...
class MismatchingParentNamesError : public std::invalid_argument {
 public:
  MismatchingParentNamesError(
    const std::string& object_to_initialize, const std::string& param1_name,
    const std::string& param1_parent_name, const std::string& param2_name,
    const std::string& param2_parent_name, const errors::ErrorTrace& trace
  )
    : std::invalid_argument(format_message(
        object_to_initialize, param1_name, param1_parent_name, param2_name,
        param2_parent_name, trace
      )) {}
...
#define THROW_MISMATCHING_PARENT_NAMES(                                 \
  ...
  throw MismatchingParentNamesError(                                    \
    object_to_initialize, param1_name, param1_parent_name, param2_name, \
    param2_parent_name, ERROR_TRACE                                     \
  )

class EmptyParentNameError : public std::invalid_argument {
 public:
  EmptyParentNameError(
    const std::string& object_to_initialize, const std::string& param_name,
    const errors::ErrorTrace& trace
  )
...
class InvalidArrayKeysError : public std::invalid_argument {
 public:
  InvalidArrayKeysError(
    const std::string& object_to_initialize, int index,
    const std::vector<std::string>& item_names, const errors::ErrorTrace& trace
  )
    : std::invalid_argument(
...
class ChildParentNameMismatchError : public std::invalid_argument {
 public:
  ChildParentNameMismatchError(
    const std::string& object_to_initialize, const std::string& child_name,
    const std::string& child_parent_name, const std::string& parent_name,
    const errors::ErrorTrace& trace
  )
    : std::invalid_argument(format_message(
        object_to_initialize, child_name, child_parent_name, parent_name, trace
      )) {}
