...
using SearchParamFilters = miru::params::SearchParamFilters;
using SearchParamFiltersBuilder = miru::params::SearchParamFiltersBuilder;
...
class ROS2StyleParameterQuery {
 public:
  ...
  get_parameter(const std::string & name, ParameterT & parameter) const {
    ...
    return miru::params::get_parameter(root_, filters, parameter);
  }
  ...
  get_parameter_or(
    ...
    SearchParamFilters filters = SearchParamFiltersBuilder().with_param_name(name).build();
    return miru::params::get_parameter_or(root_, filters, parameter, alternative_value);
  }
  ...
  get_parameter_or(
    ...
    const ParameterT & alternative_value) const {
    SearchParamFilters filters = SearchParamFiltersBuilder().with_param_name(name).build();
    return miru::params::get_parameter_or(root_, filters, alternative_value);
  }
...
// https://github.com/ros2/rclcpp/blob/a0a2a067d84fd6a38ab4f71b691d51ca5aa97ba5/rclcpp/include/rclcpp/node.hpp#L909
  ...
  get_parameters(
    ...
    std::vector<Parameter> parameters = miru::params::get_parameters(root_, filters);
