...
Config Config::from_file(
    ...
    std::variant<nlohmann::json, YAML::Node> config_data = config_file.read_structured_data();
...
Config Config::from_agent(const std::string& schema_file_path) {
    ...
    openapi::HashSchemaRequest config_schema {
        schema_file.read_string()
    };
    std::string config_schema_digest = client.hash_schema(config_schema);
    builder.with_schema_digest(config_schema_digest);
    nlohmann::json config_data = client.get_concrete_config(
