...
Config Config::from_file(
    ...
    nlohmann::json schema_content = schema_file.read_json();

    // extract the config slug
    std::string config_slug = schema_content["config_slug"];
    ...
    nlohmann::json config_data = config_file.read_json();
    builder.with_config_file(config_file);
    builder.with_data(config_data);
...
Config Config::from_agent(const std::string& schema_file_path) {
    ...
    std::string config_schema_digest = client.hash_schema(schema_content);
    builder.with_schema_digest(config_schema_digest);
    nlohmann::json config_data = client.get_concrete_config(config_schema_digest, config_slug);
    builder.with_data(config_data);
...
Config ConfigBuilder::build() {
    ...
    if (source_ == ConfigSource::Agent && !schema_digest_.has_value()) {
        throw std::runtime_error("Schema not set (must be set when retrieving the config from the agent)");
    }
    if (source_ == ConfigSource::FileSystem && !config_file_.has_value()) {
        throw std::runtime_error("Config file not set (must be set when retrieving the config from the file system)");
    }
    return Config(
        *schema_file_,
        *config_slug_,
