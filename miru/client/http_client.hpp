...
class HTTPClient {
 public:
  ...
  http::request<http::string_body> build_request(const http::verb& method,
                                                 const std::string& path,
                                                 const std::string& body = "");

  http::request<http::string_body> build_get_request(const std::string& path);
  http::request<http::string_body> build_post_request(const std::string& path,
                                                      const std::string& body);

  nlohmann::json parse_json_response(const http::response<http::string_body>& res);

  template <typename Stream>
  void send_sync_request(Stream& stream, const http::request<http::string_body>& req,
...
template <typename Stream>
void HTTPClient::send_sync_request(Stream& stream,
