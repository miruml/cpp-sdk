...
nlohmann::json TagList::to_json() const
{
    nlohmann::json j;
...
TagList TagList::from_json(const nlohmann::json& j)
{
    TagList o{};
...
bool TagList::validate(std::stringstream& msg) const
{
    return validate(msg, "");
}

bool TagList::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "TagList" : pathPrefix;

                                     
    
    /* data */ {
        const std::vector<org::openapitools::server::model::Tag>& value = data;
